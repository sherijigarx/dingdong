from lib.hashing import process_miner_music, load_hashes_to_cache, check_duplicate_music, save_hash_to_file
...
class MusicGenerationService(AIModelService):
    ...
    def handle_music_output(self, axon, music_output, prompt, model_name):
        try:
            ...
            if check_duplicate_music(audio_hash):
                ...
                bt.logging.info(f"Music processed and saved successfully for miner: {axon.hotkey}")
    ...
    def get_filtered_axons_from_combinations(self):
        ...
        if self.combinations:
            ...
            bt.logging.info(f"Current Combination for TTM: [72,73]")
            filtered_axons = [self.metagraph.axons[i] for i in [72,73]]
        else:
            self.get_filtered_axons()
            current_combination = self.combinations.pop(0)
            bt.logging.info(f"Current Combination for TTM: [72,73]")
